...
- `installOpenCode()` runs two sequential commands: install deps (`apk add nodejs npm`), then `npm install -g opencode-ai`. Accepts an optional `onProgress` callback — when provided the commands execute via `executeVerbose()` (real-time stdout streaming) instead of the blocking `execute()`; callers that don't need live feedback (tests) call without args and use the backward-compatible `execute()` path. On failure, throws `Error` with distinct prefixes — `"Installation failed (deps): "` or `"Installation failed (opencode): "` — followed by the captured error message and command output.

...
- `startServer()` launches `opencode serve ...` via `Executor.BackgroundExecutor.start()` which returns a UUID and does not reap the session on stdout EOF. The UUID is stored in a module-level `serverUuid` variable. After `STARTUP_CHECK_DELAY` (500ms), it verifies the process is alive via both `BackgroundExecutor.isRunning(uuid)` and `PROCESS_CHECK_COMMAND`. If the process exited immediately (missing binary, config error, port conflict), it throws an `Error` before the caller ever hits `waitForReady()`.
...
- `stopServer()` calls `Executor.BackgroundExecutor.stop(serverUuid)` to gracefully stop the server process. Polls `isServerUp()` for up to `STOP_POLL_TIMEOUT`; if still up, falls back to `pkill -9` (SIGKILL) and polls again. Throws `Error` if port is still occupied after SIGKILL. Resets `serverUuid = null` on successful shutdown.

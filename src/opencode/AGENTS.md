...
- `stopServer()` sets `shuttingDown = true`, runs SIGTERM via `pkill -f "opencode serve"`, polls `isServerUp()` for up to `STOP_POLL_TIMEOUT`, escalates to SIGKILL (`pkill -9`) if needed, and polls again. Resets `shuttingDown = false` in `finally`. Throws `Error` if port is still occupied after SIGKILL.

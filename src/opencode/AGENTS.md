...
- `startServer()` fires a `nohup opencode serve ... &` command via `execute()`, then waits `STARTUP_CHECK_DELAY` (500ms) and validates the process is alive via `PROCESS_CHECK_COMMAND`. If the process exited immediately (missing binary, config error, port conflict), it throws an `Error` before the caller ever hits `waitForReady()`. The caller must not call `execute()` directly for server start.

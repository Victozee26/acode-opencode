...
- `app.ts` — legacy global debug master switch: `DEBUG` (kept for backward compat; runtime control is via `enableConsoleLogs` setting).
- `settings.ts` — iframe scale, auto-start, console-logs enable, log-level, and hide-header-in-landscape setting constants: `SETTINGS_KEY_IFRAME_SCALE`, `DEFAULT_IFRAME_SCALE`, `IFRAME_SCALE_MIN`, `IFRAME_SCALE_MAX`, `IFRAME_SCALE_STEP`, `SETTINGS_KEY_AUTO_START`, `DEFAULT_AUTO_START`, `SETTINGS_KEY_ENABLE_CONSOLE_LOGS`, `DEFAULT_ENABLE_CONSOLE_LOGS`, `SETTINGS_KEY_LOG_LEVEL`, `DEFAULT_LOG_LEVEL`, `SETTINGS_KEY_HIDE_HEADER_IN_LANDSCAPE`, `DEFAULT_HIDE_HEADER_IN_LANDSCAPE`.

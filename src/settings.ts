  ...
  return {
    list: [
      ...
      },
      {
        ...
        text: 'Log Level',
        info: 'Verbosity of plugin log output (none disables all logs, debug < info < warn < error)',
        select: ['none', 'debug', 'info', 'warn', 'error'] as const,
    ...
    cb(_key: string, value: unknown) {
      if (_key === SETTINGS_KEY_IFRAME_SCALE) {
        ...
        if (['none', 'debug', 'info', 'warn', 'error'].includes(strVal)) {
          cachedLogLevel = strVal;
          persist(_key, strVal);
          setLogLevel(strVal as 'none' | 'debug' | 'info' | 'warn' | 'error');

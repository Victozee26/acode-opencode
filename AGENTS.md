...
- `src/main.ts`, `src/types.ts`, `src/state.ts`, `src/logger.ts`, `src/error.ts`, `src/settings.ts` — Cross-cutting infrastructure owned directly by root AGENTS.md. `main.ts` (`AcodePlugin`) init reads `getEnableConsoleLogs()` → `setLogEnabled` + `getLogLevel()` → `setLogLevel` (runtime log gate, default off via `enableConsoleLogs` native setting); wires live UI handlers: `setOnScaleChange` → `updateIframeScale`, `setOnHideHeaderChange` → `applyHeaderVisibility` (global chrome, landscape gated via `body` class; see `src/ui/AGENTS.md`), and tears down orientation listeners via `destroyOrientationListener()` on `destroy()` (idempotent, no duplicate registration on re-init). `settings.ts` owns the native plugin settings schema (`getSettingsSchema`) and live `acode.require('settings')` getters plus `setOn*Change` callbacks for `iframeScale`, `enableConsoleLogs` (→ `setLogEnabled`, full off switch), `logLevel` (→ `setLogLevel`), and `hideHeaderInLandscape`. Auto-start setting removed; plugin auto-starts on page show when not already running.

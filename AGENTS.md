...
- `src/main.ts`, `src/types.ts`, `src/state.ts`, `src/logger.ts`, `src/error.ts`, `src/settings.ts` — Cross-cutting infrastructure owned directly by root AGENTS.md. `main.ts` (`AcodePlugin`) wires live UI handlers: `setOnScaleChange` → `updateIframeScale`, `setOnHideHeaderChange` → `applyHeaderVisibility` (global chrome, landscape gated via `body` class; see `src/ui/AGENTS.md`), and tears down orientation listeners via `destroyOrientationListener()` on `destroy()` (idempotent, no duplicate registration on re-init).
